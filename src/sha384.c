...
static void					sha384_init_string(
		unsigned long *tmp_arr, const unsigned char *string)
{
	int i;
	int j;
...
static void				sha384_transform_loop(unsigned long *temp, unsigned long *tmp_arr)
{
	unsigned long	t1;
	unsigned long	t2;
	int 			i;
		...
		unsigned long *state, const unsigned char *string)
{
	...
	sha384_init_string(tmp_arr, string);
	i = -1;
	while (++i < 8)
		temp[i] = state[i];
	i = -1;
	sha384_transform_loop(temp, tmp_arr);
	while (++i < 8)
		state[i] += temp[i];
}

unsigned char	*sha384_string_pad(t_content *string)
{
	size_t			bits;
	size_t			done_len;
...
void	sha384(t_content *string)
{
	...
	string->content = sha384_string_pad(string);
	ft_memdel((void **)&to_free);
	i = 0;
	while (i < string->cont_len)
	{
		sha384_process(state, string->content + i);
		i += 1024 / 8;
	}
	i = 0;
	while (i < 6)
