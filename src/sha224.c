...
unsigned char	*msg_padding_224(t_content *string)
{
	size_t			bits;
	size_t			done_len;
...
static void				sha224_init_msg_schedule(
		...
		unsigned long *state, const unsigned char *data)
{
	...
	sha224_init_msg_schedule(m, data);
	i = -1;
	while (++i < 8)
...
void		sha224(t_content *string)
{
	...
	string->content = msg_padding_224(string);
	ft_memdel((void **)&to_free);
	i = 0;
	while (i < string->cont_len)
	{
		sha224_transform(state, string->content + i);
		i += 512 / 8;
	}
	i = 0;
	while (i < 7)
