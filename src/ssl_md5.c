...
static unsigned int	*md5_init_hash_values(void)
{
...
void				ssl_md5(t_word *word, t_flags *flags)
{
	...
	s_arr = md5_init_s_arr();
	k_arr = md5_init_k_arr();
	hash_values = md5_init_hash_values();
	hash_values = md5_start_processing(word, hash_values, s_arr, k_arr);
	print_result(hash_values, flags);
	free(s_arr);
	free(k_arr);
	free(hash_values);
}
