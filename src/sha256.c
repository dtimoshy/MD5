...
static void				sha256_init_string_loop(
		...
		unsigned long *state, const unsigned char *string)
{
	...
	sha256_init_string_loop(tmp_arr, string);
	i = -1;
	while (++i < 8)
		temp[i] = (unsigned int)state[i];
	sha256_transform_loop(temp, tmp_arr);
	i = -1;
	while (++i < 8)
		state[i] += temp[i];
}
...
void					sha256(t_content *string)
{
	...
	string->content = sha256_string_pad(string);
	ft_memdel((void **)&to_free);
	i = 0;
	while (i < string->cont_len)
	{
		sha256_process(state, string->content + i);
		i += 512 / 8;
	}
	i = 0;
	while (i < 8)
